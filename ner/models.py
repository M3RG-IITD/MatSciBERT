...
class BERT_CRF(nn.Module):
    def __init__(self, model_name, device, config, cache_dir):
        ...
        self.encoder = AutoModelForTokenClassification.from_pretrained(model_name, from_tf=False, config=config, 
                                                                       ...
                                                                       use_auth_token=None)
        self.crf = BIO_Tag_CRF(num_tags=config.num_labels, batch_first=True)
    ...
    def forward(self, **inputs):
        ...
        if self.training:
            return (-self.crf(padded_logits, padded_labels, mask=padded_attention_mask, reduction='mean'), )
        else:
            out = self.crf.decode(padded_logits, mask=padded_attention_mask)
            ...
            for i in range(len(out)):
                ...
                for j in range(len(labels[i])):
                    ...
                    k += 1
                assert(k == len(out[i]))
            return (out_logits.sum(), out_logits, )
