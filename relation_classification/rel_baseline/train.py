...
import time; import argparse; import copy; import params; from tqdm import tqdm
...
def macro_micro_f1(y_true, y_pred):
    label_list = list(np.arange(max(y_true)+1))
    report = classification_report(y_true, y_pred, labels=label_list, output_dict=True, zero_division=0)
    results = dict()
    results['macro_f1'] = report['macro avg']['f1-score']
    results['micro_f1'] = f1_score(y_true, y_pred, labels=label_list, average='micro', zero_division=0)
    return results
...
def train(model, iterator, optimizer, criterion, epoch, valid_iterator):
    ...
    for batch in iter_func(iterator):
        ...
        y_pred += predictions.argmax(dim=1).tolist()
        correct, tot = accuracy(predictions, batch.label)
        loss.backward()
        optimizer.step()
        epoch_loss += loss.item()

    return epoch_loss / len(iterator), macro_micro_f1(y_true, y_pred)
...
for epoch in range(epoch_initial, args.epochs+epoch_initial):
    ...
    train_loss, train_res = funct(model, train_iterator, optimizer, criterion, epoch , valid_iterator)
    valid_loss, valid_res = funce(model, valid_iterator, criterion)
    if valid_res['macro_f1'] < best_valid_macro_f1:
        patience +=1
    else:
        patience = 0
    ...
    if valid_res['macro_f1'] > best_valid_macro_f1:
        best_valid_macro_f1 = valid_res['macro_f1']
        torch.save({
    ...
    myprint(f'\tTrain Loss: {train_loss:.3f} | Train Res: {train_res}')
    myprint(f'\t Val. Loss: {valid_loss:.3f} |  Val. Res: {valid_res}')

    train_acc_f.write(f'MacroF1: {train_res["macro_f1"]}, MicroF1: {train_res["micro_f1"]}\n')
    valid_acc_f.write(f'MacroF1: {valid_res["macro_f1"]}, MicroF1: {valid_res["micro_f1"]}\n')
    train_acc_f.flush()
    valid_acc_f.flush()
...
val_loss, val_res = evaluate(model, valid_iterator, criterion)
myprint(f'Val. Loss: {val_loss:.3f} | Val. Res: {val_res}')
valid_acc_f.write(f'MacroF1: {val_res["macro_f1"]}, MicroF1: {val_res["micro_f1"]}\n')
valid_acc_f.flush()
valid_acc_f.close()

test_loss, test_res = evaluate(model, test_iterator, criterion)
myprint(f'Test Loss: {test_loss:.3f} | Test Res: {test_res}')
test_acc_f.write(f'MacroF1: {test_res["macro_f1"]}, MicroF1: {test_res["micro_f1"]}\n')
test_acc_f.flush()
test_acc_f.close()
